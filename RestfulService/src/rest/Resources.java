...
public class Resources {
    ...
    public Response sendAllEmployes() {
        ...
        if (Json == null)
            return Response.status(Errors.status_generic_error).entity(Errors.generic_error).build();
    ...
    public Response sendEmployee(@PathParam("id") String id) throws SQLException {
        ...
        if (temp == null)
            return Response.status(Errors.status_not_found).entity(Errors.not_found).build();
    ...
    public Response newEmployee(String employee) {
        ...
        if (temp == null)
            return Response.status(Errors.status_invalid_input).entity(Errors.invalid_input).build();
    ...
    public Response createNewLocal(String json) {
        ...
        if (result == -1)
            return Response.status(Errors.status_existing_key).entity(Errors.existing_key).build();
        else if (result == -2)
            return Response.status(Errors.status_generic_error).entity(Errors.generic_error).build();
